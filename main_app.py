...
import step_05_outlier_treatment # Step 5 모듈 임포트
import traceback
import hashlib
import json
import datetime
...
class AppState:
    def __init__(self):
        ...
        self.df_after_step5 = None # Step 5 결과 DataFrame 추가
...
def step1_processing_complete(processed_df: pd.DataFrame):
    ...
    app_state.df_after_step5 = None # Reset Step 5
...
def step5_outlier_treatment_complete(processed_df: pd.DataFrame):
    ...
    # If there's a step 6, trigger its update here
    # if len(ANALYSIS_STEPS) > 5 and ANALYSIS_STEPS[5] in app_state.module_ui_updaters:
...
def load_data_from_file(file_path: str) -> bool:
    ...
    try:
        ...
        app_state.df_after_step3 = None
        app_state.df_after_step4 = None
        app_state.df_after_step5 = None # Reset Step 5 df
...
def reset_application_state(clear_df_completely=True):
    ...
    if hasattr(step_05_outlier_treatment, 'reset_outlier_treatment_state'): step_05_outlier_treatment.reset_outlier_treatment_state() # Reset Step 5
...
def export_to_parquet_callback():
    ...
    if app_state.df_after_step5 is not None: # Check Step 5 first
...
def gather_current_settings() -> dict:
    ...
    settings = {
        ...
        'step_05_outlier_treatment_settings': {}, # Step 5 설정 추가
    }
    ...
    if hasattr(step_05_outlier_treatment, 'get_outlier_treatment_settings_for_saving'): # Step 5 설정 저장
...
def apply_settings(settings_dict: dict):
    ...
    if STEP_03_SAVE_LOAD_ENABLED:
        ...
        # else: # If no settings or df_after_step1 is None, still call complete to ensure flow
        #    main_app_callbacks['step3_processing_complete'](app_state.df_after_step1.copy() if app_state.df_after_step1 is not None else None)
    # else: 
    #    main_app_callbacks['step3_processing_complete'](app_state.df_after_step1.copy() if app_state.df_after_step1 is not None else None)
    ...
    # else:
    #    main_app_callbacks['step4_missing_value_processing_complete'](df_input_for_step4.copy() if df_input_for_step4 is not None else None)

    app_state.df_after_step5 = None # Step 5 df reset
    ...
    # else: # Ensure the callback is called even if settings are not applied, to maintain data flow
    #    main_app_callbacks['step5_outlier_treatment_complete'](df_input_for_step5.copy() if df_input_for_step5 is not None else None)
...
main_app_callbacks = {
    ...
    'get_df_after_step5': lambda: app_state.df_after_step5, # Step 5 df getter
    ...
    with dpg.group(horizontal=True, tag="main_layout_group"): # 전체를 감싸는 가로 그룹
        ...
        with dpg.child_window(tag="content_area", border=True, parent="main_layout_group", width=content_area_width):
            for step_name_create in ANALYSIS_STEPS:
                ...
                elif step_name_create == ANALYSIS_STEPS[4]: # Step 5 UI 생성
